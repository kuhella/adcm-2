...
$adcmvenv/python "${scripts}"/manage_secrets.py migrate || exit $?

if [ -z "$MIGRATION_MODE" ] || [ "$MIGRATION_MODE" -ne 1 ]; then
    $adcmvenv/python "${scripts}"/manage_secrets.py init || exit $?
fi

$adcmvenv/python "${django_command}" compatibility_check || exit $?
