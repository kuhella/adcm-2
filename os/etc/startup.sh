...
if [ "$is_in_mm" -ne 1 ]; then
  ...
  # Each /etc/sv/<svc>/supervise is a symlink into the per-user runtime dir
  # (/run/user/<uid>/runit/<svc>, see Dockerfile) so /etc/sv and /run stay
  # read-only / root-owned; create the writable targets first.
  runit_base="${run}/runit"
