...
# Install Python 3.12 and 3.10 (3.10 is needed for Ansible 2.16 venv)
...
# 1. Copy base venv and Python 3.10 runtime from external Ansible image
...
RUN --mount=from=ghcr.io/astral-sh/uv,source=/uv,target=/bin/uv \
    ...
    uv pip install --python /venv/2.16/bin/python -r ansible-2.16-python3.10-dependencies.txt
